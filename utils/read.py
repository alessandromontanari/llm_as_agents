...
class BaseType(object):
    ...
    def get_template_qa(self, entry, random_seed):
        """
        random sample a qa template for each entry
        """
        random.seed(random_seed)  # fix random seed for reproduction
        template = random.choice(self.get_all_templates_qa(entry, random_seed))
        return template
    ...
    def fill_in_the_template_qa(self, template, kw_dic):
        question = template[0].format(**kw_dic)
        answer = template[1].format(**kw_dic)
        return [question + self.qa_deliminator, answer]
...
class nli(BaseType):
    ...
    def format_two_demos(self, entry, random_seed):
        """
        Split the output in question and answer
        """
        kw_dic = {}
        kw_dic['premise'] = entry["premise"]
        hypothesis = entry["hypothesis"]
        ...
        template = self.get_template(entry, random_seed)
        return self.fill_in_the_template_qa(template, kw_dic)
    ...
    def mine(self, text, domain, sents, **kwargs):
        ...
        for sent in sents:
            ...
            if len(specific_tokens_in_sent) >= self.min_kw_num and len(sent_tokens) <= self.max_sent_len:
                tokens = [self.domain_spm.decode(token) for token in
                          specific_tokens_in_sent]  # transfer tokens back to normal words
                dic = {
                    'relation': 'word2text',
                    'domain': domain,
                    'token_set': tokens,
...
class summarize(BaseType):
    ...
    def get_all_templates_qa(self, entry, random_seed):
        ...
        return [
            # summary_templates
            ["{context_wo_title}\n\nWhat is a potential title for this context in the {domain} domain?", "Title: {title}"],
            ["{domain} article: {context_wo_title}{qa_demos}\n\nWhat is the title of this article?", "{title}"],
            ["Article: {context_wo_title}{qa_demos}\n\nGenerate a title for this {domain} paragraph.", "Title: {title}"],
            ["{context_wo_title}\n\nWrite a title for the above {domain} article.", "{title}"],
            ["{context_wo_title}\nBriefly summarize this {domain} text?", "{title}"],
            ["Article in the {domain} domain: {context_wo_title}\n\nGenerate a short summary for this article.", "Answer: {title}"],
            ["{context_wo_title}{qa_demos}\n\nSummarize the aforementioned {domain} text in a single sentence.", "{title}"],
            ["{context_wo_title}\nCan you generate a short summary of the above {domain} paragraph?", "{title}"],
            ["{context_wo_title}\nPlease write a short summary for the above article in the {domain} domain.", "{title}"],
            ["Context: {context_wo_title}{qa_demos}\n\nWhat was this {domain} article about?", "{title}"],
            # write based on title
            ["Write an article about {domain} domain, using the following title: {title}.", "Article: {context_wo_title}"],
            ["Title: {title}\nWrite a an article about {domain} domain based on this title.", "{context_wo_title}"],
            ["Use the title \"{title}\" to write a {domain} article.", "Article: {context_wo_title}"],
            ["Craft an informative article about the {domain} domain, drawing from the following summary: {title}", "Article: {context_wo_title}"],
            ["Create a {domain} article inspired by the provided title: {title}", "Output: {context_wo_title}"],
            ["Can you develop an engaging {domain} article using the title \"{title}\"?", "{context_wo_title}"],
            ["Write an informative piece on the {domain} domain, using the provided title: {title}.", "{context_wo_title}"],
            ["Craft an article focused on {domain}, utilizing the provided title: {title}.", "Article: {context_wo_title}"],
            ["Compose an in-depth {domain} article based on the title: {title}", "Article: {context_wo_title}"],
            ["Can you create an article delving into the {domain} domain, incorporating the given title \"{title}\"?", "{context_wo_title}"],
        ]

    def format_single_demo(self, entry, random_seed):
        sents = entry.pop('sents')
        template = self.get_template(entry, random_seed)
        template_qa = self.get_template_qa(entry, random_seed)

        entry['context_wo_title'] = ''.join(sents).strip()
        final_demo = template.format(**entry)
        final_demo_qa = [template_qa[0].format(**entry), template_qa[1].format(**entry)]

        return final_demo, final_demo_qa
...
class text_completion(BaseType):
    ...
    def get_all_templates_qa(self, entry, random_seed):
        ...
        return [
            ["Please complete an article about {domain}: {context_1st_half}",  "{context_2nd_half}"],
            ["Here is the first part of an article about {domain}: {context_1st_half}\n\nPlease complete it.", "Completion: {context_2nd_half}"],
...
class summarize_completion(BaseType):
    ...
    # TODO: Question: and Task:, and Answer:, Summary:, Title: as keywords for the splitting

    def get_all_templates_qa(self, entry, random_seed):
        # applicable to both text completion and summarization:
        return [
            ["Please complete an article about {domain}: {context_1st_half}\n\nQuestion: What was this article about?",  "{context_2nd_half}\n\nAnswer: {title}"],
            ["Here is the first part of an article about {domain}: {context_1st_half}\n\nPlease complete it.\n\nQuestion: What was this article about?", "Completion: {context_2nd_half}\n\nAnswer: {title}"],
            ["Explore the initial section of an article on {domain}: {context_1st_half}\n\nProvide the text ending?\n\nQuestion: Propose a title for this context?", "{context_2nd_half}\n\nAnswer: {title}"],
            ["Read the beginning of an article about {domain}: {context_1st_half}\n\nYour task is to add the subsequent part.\n\nTask: Briefly summarize this text.", "{context_2nd_half}\n\nSummary: {title}"],
            ["In this article snippet about {domain}, you will find the first half: {context_1st_half}\n\nCompose the remaining section\n\nTask: Write a title for it.", "{context_2nd_half}\n\nTitle: {title}"],
            ["Take a look at the first part of an article on {domain}: {context_1st_half}\n\nYour challenge is to write the following segment.\n\nQuestion: What is a very short summary of the above text?", "{context_2nd_half}\n\nAnswer: {title}"],
            ["Review the initial portion of an article discussing {domain}: {context_1st_half}\n\nWhat would you include in the rest of the article?\n\nQuestion: What is a shorter version of this article?", "{context_2nd_half}\n\nAnswer: {title}"],
            ["Consider the opening of an article centered around {domain}: {context_1st_half}\n\nNow, provide the continuation of the article.\n\nQuestion: What was this article about?", "Continuation: {context_2nd_half}\n\nAnswer: {title}"],
            ["Examine the first segment of an article exploring {domain}: {context_1st_half}\n\nComplete the article?\n\nQuestion: Can you generate a short summary of the above paragraph?", "{context_2nd_half}\n\nAnswer: {title}"],
            ["Read the beginning of an article on {domain}: {context_1st_half}\n\nHow would you extend the article?\n\nTask: Please write a short summary for the above article.", "{context_2nd_half}\n\nSummary: {title}"]
        ]
...
class no_summarize_completion(BaseType):
    ...
    def get_all_templates_qa(self, entry, random_seed):
        return [
            ["Please answer some questions about the following article in the {domain} domain:\n{context}{q_demos}", "{a_demos}"],
            ["Read this {domain} article and answer questions\n{context}{q_demos}", "{a_demos}"],
            ["{context}{q_demos}", "{a_demos}"],
            ["Answer some questions about this article about {domain} domain:\n{context}{q_demos}", "{a_demos}"],
            ["Here are some questions about this {domain} article: {context}{q_demos}", "{a_demos}"],
            ["{domain} article: {context}{q_demos}", "{a_demos}"],
            ["Article: {context}{q_demos}", "{a_demos}"],
            ["Read this article in the {domain} domain: {context}{q_demos}", "{a_demos}"],
            ["Given the {domain} article: {context}{q_demos}", "{a_demos}"],
            ["Context: {context}{q_demos}", "{a_demos}"],
            ["Article in the {domain} domain: {context}{q_demos}", "{a_demos}"],
            ["Use this {domain} article to answer the questions: {context}{q_demos}", "{a_demos}"],
            ["Answer based on context in the {domain} domain:\n{context}{q_demos}", "{a_demos}"],
        ]
...
class overall(BaseType):
    ...
    def format_recomprehension(self, overall_entry, insert_types=TYPES):
        ...
        for type in list(set(insert_types) & set(['nli', 'common_reason', 'paraphrase', 'word2text'])):
            ...
            qa_demos_list.append([type_cls.format_two_demos(example, seed) for example in type_examples])
        ...
        if len(qa_demo_list) > 0:
            ...
            for demo_task in qa_demos_list[0]:
                q_demos.append(f'{intro}{self.demo_deliminator.join([demo_task[0]])}')
                a_demos.append(demo_task[1])
        else:
            qa_demos = ''
            q_demos = ''
            a_demos = ''

        def summarize_only(count_dict):
            ...
            entry['q_demos'] = q_demos
            entry['a_demos'] = a_demos
            entry['spm'] = self.ori_spm
            read_compre_demo, read_compre_demo_qa = overall_cls.format_single_demo(entry, seed)
            for index, element in enumerate(read_compre_demo_qa):
                read_compre_demo_qa[index] = remove_double_space(element)
            return remove_double_space(read_compre_demo), read_compre_demo_qa, count_dict
